...
var platform_Enemy_Collectible = preload("res://Scenes/platform_Enemy_Collectible.tscn")
...
func _process(_delta):
	...
	# Update the distance points and score based on world_speed
	distance_points += world_speed * _delta
	if distance_points >= 100:  # Change this value to control how often points are awarded based on distance
		distance_points -= 25
		score += 1
...
func add_score(value):
	score += value
	if score % 5 == 0:  # Increase world speed every 5 points
		world_speed += 15  # Adjust this value to control the speed increment
